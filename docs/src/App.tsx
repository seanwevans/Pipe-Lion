...
/// Captures served from `public/samples`, so a first visit has something to
/// open. Generated by `tools/make_sample_captures.py`; each one exercises a
/// different decoder. Keep the file names in step with that script.
const SAMPLE_CAPTURES = [
  { file: "dns-lookup.pcap", label: "DNS lookup" },
  { file: "tcp-http.pcap", label: "TCP / HTTP" },
  { file: "tls-client-hello.pcap", label: "TLS Client Hello" },
  { file: "icmp-ping.pcap", label: "ICMP ping (IPv4 + IPv6)" },
  { file: "mixed-traffic.pcapng", label: "Mixed traffic (pcapng)" },
] as const;
...
function App() {
  ...
  const onSampleChange = useCallback(
    async (event: React.ChangeEvent<HTMLSelectElement>) => {
      ...
      // Reset immediately so picking the same sample twice fires again.
      event.target.value = "";
      const sample = SAMPLE_CAPTURES.find(({ file }) => file === fileName);
      if (!sample) {
        return;
      }

      setStatus(`Fetching ${sample.label} sample…`);
      try {
        // BASE_URL keeps this correct under a project Pages path.
        ...
        setError(
          `Could not load the ${sample.label} sample: ${
      ...
      <div className="window">
        ...
        <div className="toolbar">
          <div className="toolbar-buttons">
            ...
            <label className="toolbar-select" htmlFor="sample-capture">
              <span>Sample</span>
              <select
                id="sample-capture"
                defaultValue=""
              ...
              >
                ...
                {SAMPLE_CAPTURES.map(({ file, label }) => (
            ...
            >
              ...
              {hasPacketData ? (
                ...
                <div className="table-row empty" role="row">
                  ...
                  <span role="cell" className="info-cell">
                    No capture handy? Pick one from the Sample menu above.
                  </span>
                </div>
              )}
            </div>
